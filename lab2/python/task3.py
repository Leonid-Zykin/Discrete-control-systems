...
N = 200
x = np.array([1.0, 0.0])  # начальное состояние объекта (по заданию x(0)≠0)
...
for k in range(N):
    ...
    # Обновление внутренней модели (по ошибке, как в задании 2)
...
# Оценим ошибку после 4-го такта (t>=3 c)
settle_idx = int(np.ceil(3.0 / T))
post_err = max(abs(e) for e in e_hist[settle_idx:]) if settle_idx < len(e_hist) else float('nan')
print(f'Макс |e| для t>=3 c: {post_err:.6e} (k>={settle_idx})')
